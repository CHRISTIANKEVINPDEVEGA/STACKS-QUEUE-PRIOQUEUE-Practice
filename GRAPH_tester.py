...
def sorting(neighbors, approach):
    return sorted(neighbors.items(), key = lambda item: approach(item[1]))

def distance_length(node_weights):
    return float(node_weights["distance"])

for neighbor, node_weights in sorting(graph[nodes["york"]], distance_length):
    print(f"{node_weights['distance']:>3} miles, {neighbor.name}")
